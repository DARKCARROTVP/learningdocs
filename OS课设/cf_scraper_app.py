...
def fetch_and_extract_page(page_num: int):
    ...
    try:
        with sync_playwright() as p:
            ...
            page.wait_for_selector("div.datatable.ratingsDatatable", timeout=20000)
...
def scrape_codeforces_concurrent(start_page=1, end_page=10, max_threads=5, progress_callback=None):
    ...
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        futures = {
            executor.submit(fetch_and_extract_page, page)
    ...
    # === thread_id 统一映射为连续编号（1 开始）===
    raw_ids = list({t["thread_id"] for t in thread_timings if "thread_id" in t})
    thread_id_map = {tid: idx + 1 for idx, tid in enumerate(sorted(raw_ids))}
    for t in thread_timings:
        if "thread_id" in t:
            t["thread_id"] = thread_id_map[t["thread_id"]]
